					...
					<div className={isOpenBurgerMenu ? styles.BurgerMenu : styles.BurgerMenuClosed}>
						<div className={styles.Content}>
							<div className={styles.Navs}>
								{
									Navs.map((Nav, index) => <Link key={index} className={styles.Nav} href={Nav.href} onClick={() => setIsOpenBurgerMenu(false)}>{Nav.link}</Link>)
								}
							</div>
							...
							<div className={styles.Contacts}>
								{
									Contacts.map((Contact, index) => <div key={index} className={styles.Contact}><Image src={Contact.icon} alt='icon' width={25} height={25} /><p className={styles.TextContact}>{Contact.title}</p></div>)
								}
							</div>
						</div>
					</div>
